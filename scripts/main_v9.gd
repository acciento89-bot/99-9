...
func _share_result_value(value: float, streak_value: int) -> void:
    ...
    # kyoz/godot-share iOS plugin (Godot 4.7 release): native
    # UIActivityViewController on both iPhone and iPad.
    if Engine.has_singleton("Share"):
        ...
        if native_share != null:
            if native_share.has_method("shareImage") and not image_path.is_empty():
                ...
                shared = true
            # Also tolerate the snake_case API used by newer share wrappers.
    ...
    # Safe fallback for editor/non-iOS builds. The release iOS build is expected
    # to have the native Share singleton and therefore never use this path.
