...
KEY_DIR="$RUNNER_TEMP/shk-build3-review-v3"
mkdir -p "$KEY_DIR"
...
find_submission_by_states() {
  ...
  FOUND_SID=$(jq -r --arg vid "$version_id" --arg states "$states" '
    ($states|split(",")) as $s |
    [.data[]? | select(.attributes.state as $st | $s | index($st)) | select(.relationships.appStoreVersionForReview.data.id==$vid)] | .[0].id // empty' "$out")
  if [[ -z "$FOUND_SID" ]]; then
    local count
...
submission_contains_version() {
  local sid="$1" out="$RUNNER_TEMP/$key-items-$sid.json" code
...
for key in kaltecalc lueftungscalc heizkoerpercalc rohrcalc anlagencheck; do
  ...
  echo; echo "========== $name: Build 3 review replacement v3 =========="

  f="$RUNNER_TEMP/$key-app.json"; code=$(raw_get "/v1/apps?filter%5BbundleId%5D=$bundle&limit=10" "$f"); require_code 200 "$code" "$f" "$name: find app"; app_id=$(jq -r '.data[0].id // empty' "$f"); [[ -n "$app_id" ]]
  f="$RUNNER_TEMP/$key-versions.json"; code=$(raw_get "/v1/apps/$app_id/appStoreVersions?filter%5Bplatform%5D=IOS&filter%5BversionString%5D=$VERSION&limit=50" "$f"); require_code 200 "$code" "$f" "$name: find version"; version_id=$(jq -r '.data[0].id // empty' "$f"); [[ -n "$version_id" ]]
  f="$RUNNER_TEMP/$key-build3.json"; code=$(raw_get "/v1/builds?filter%5Bapp%5D=$app_id&filter%5Bversion%5D=$TARGET_BUILD&sort=-uploadedDate&limit=10" "$f"); require_code 200 "$code" "$f" "$name: find Build 3"; build3_id=$(jq -r '.data[]? | select(.attributes.processingState=="VALID") | .id' "$f" | head -1); [[ -n "$build3_id" ]]
  ...
  if [[ "$CURRENT_BUILD_ID" != "$build3_id" && ( "$CURRENT_VSTATE" == 'WAITING_FOR_REVIEW' || "$CURRENT_VSTATE" == 'IN_REVIEW' || "$CURRENT_VSTATE" == 'WAITING_FOR_EXPORT_COMPLIANCE' ) ]]; then
    find_submission_by_states 'WAITING_FOR_REVIEW,IN_REVIEW,UNRESOLVED_ISSUES,CANCELING' "$RUNNER_TEMP/$key-active.json"
    old_sid="$FOUND_SID"
    [[ -n "$old_sid" ]] || { echo "ERROR: $name active submission missing"; exit 1; }
    body=$(jq -nc --arg id "$old_sid" '{data:{type:"reviewSubmissions",id:$id,attributes:{canceled:true}}}')
    f="$RUNNER_TEMP/$key-cancel.json"; code=$(raw_patch "/v1/reviewSubmissions/$old_sid" "$body" "$f"); require_code 200 "$code" "$f" "$name: cancel review"
    echo "CANCEL_REQUESTED=$old_sid"
    editable='false'
    for attempt in $(seq 1 48); do
      sleep 5; read_version "$RUNNER_TEMP/$key-cancel-$attempt.json"; echo "POST_CANCEL_STATE=$CURRENT_VSTATE attempt=$attempt"
      case "$CURRENT_VSTATE" in DEVELOPER_REJECTED|PREPARE_FOR_SUBMISSION|READY_FOR_REVIEW) editable='true'; break;; esac
    done
    [[ "$editable" == 'true' ]] || { echo "ERROR: $name not editable after cancel"; exit 1; }
  fi

  if [[ "$CURRENT_RELEASE_TYPE" != 'AFTER_APPROVAL' ]]; then
    body=$(jq -nc --arg id "$version_id" '{data:{type:"appStoreVersions",id:$id,attributes:{releaseType:"AFTER_APPROVAL"}}}'); f="$RUNNER_TEMP/$key-release.json"; code=$(raw_patch "/v1/appStoreVersions/$version_id" "$body" "$f"); require_code 200 "$code" "$f" "$name: release type"
  fi
  if [[ "$CURRENT_BUILD_ID" != "$build3_id" ]]; then
    body=$(jq -nc --arg id "$build3_id" '{data:{type:"builds",id:$id}}'); f="$RUNNER_TEMP/$key-attach.json"; code=$(raw_patch "/v1/appStoreVersions/$version_id/relationships/build" "$body" "$f"); [[ "$code" == '204' || "$code" == '200' ]] || { echo "ERROR: $name attach Build 3 HTTP $code"; cat "$f"; exit 1; }
  fi
  read_version "$RUNNER_TEMP/$key-after-attach.json"
  [[ "$CURRENT_BUILD_NUMBER" == '3' && "$CURRENT_BUILD_ID" == "$build3_id" ]] || { echo "ERROR: $name Build 3 not attached"; exit 1; }
  echo "BUILD3_ATTACHED=YES STATE=$CURRENT_VSTATE"

  # Wait for Apple's canceled package to become reusable. This avoids creating a second package while the version still belongs to the first.
  sid=''
  for attempt in $(seq 1 36); do
    find_submission_by_states 'READY_FOR_REVIEW' "$RUNNER_TEMP/$key-ready-$attempt.json"
    if [[ -n "$FOUND_SID" ]]; then
      if submission_contains_version "$FOUND_SID"; then sid="$FOUND_SID"; break; fi
      # appStoreVersionForReview can identify it even when the item endpoint has not caught up yet.
      related=$(jq -r --arg id "$FOUND_SID" '.data[]? | select(.id==$id) | .relationships.appStoreVersionForReview.data.id // empty' "$RUNNER_TEMP/$key-ready-$attempt.json")
  ...
  if [[ -z "$sid" ]]; then
    # Only create a fresh package after Apple has had time to release the old one.
    body=$(jq -nc --arg app "$app_id" '{data:{type:"reviewSubmissions",relationships:{app:{data:{type:"apps",id:$app}}}}}')
    f="$RUNNER_TEMP/$key-create.json"; code=$(raw_post '/v1/reviewSubmissions' "$body" "$f"); require_code 201 "$code" "$f" "$name: create review submission"; sid=$(jq -r '.data.id' "$f")
    ...
    f="$RUNNER_TEMP/$key-add.json"; code=$(raw_post '/v1/reviewSubmissionItems' "$body" "$f"); require_code 201 "$code" "$f" "$name: add version to new review submission"
  else
    echo "REUSING_REVIEW_SUBMISSION=$sid"
  fi

  body=$(jq -nc --arg id "$sid" '{data:{type:"reviewSubmissions",id:$id,attributes:{submitted:true}}}')
  f="$RUNNER_TEMP/$key-submit.json"; code=$(raw_patch "/v1/reviewSubmissions/$sid" "$body" "$f"); require_code 200 "$code" "$f" "$name: submit Build 3"

  ok='false'
  for attempt in $(seq 1 36); do
    sleep 5; read_version "$RUNNER_TEMP/$key-final-$attempt.json"; echo "FINAL_STATE=$CURRENT_VSTATE FINAL_BUILD=$CURRENT_BUILD_NUMBER attempt=$attempt"
    if [[ "$CURRENT_BUILD_NUMBER" == '3' && ( "$CURRENT_VSTATE" == 'WAITING_FOR_REVIEW' || "$CURRENT_VSTATE" == 'IN_REVIEW' ) ]]; then ok='true'; break; fi
  done
  [[ "$ok" == 'true' ]] || { echo "ERROR: $name did not return to review with Build 3"; exit 1; }
